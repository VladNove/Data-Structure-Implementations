...
main(void)
{
	...
	qstack_t *qst = qst_create(sizeof(int), 10);
	...
	qst_push(qst, &numbers[9]);
	qst_push(qst, &numbers[8]);
	qst_push(qst, &numbers[7]);
	...
	/* Test multiple pops */
	printf("#5:\nOutput: ");
	while (!qst_is_empty(qst)) {
		printf("%d ", *(int *)qst_peek(qst));
		qst_pop(qst);
	}
	printf("size: %d", qst_get_size(qst));
	printf("\nOutput asteptat: 7 8 9 size: 0\n");
