...
/*
bool
__b_tree_insert(b_node_t* b_node, b_node_t* b_node_tmp, void* data)
{
    if(!b_node_tmp->left)
    {
...
void __b_tree_free_bonus(b_tree_t* b_tree, void (*free_data)(void*))
{
    while(b_tree->root){
        b_node_t* aux = b_tree->root;
        b_node_t* prec = NULL;
        while (aux)
        {
            if (aux->left) {
...
b_tree_free(b_tree_t* b_tree, void (*free_data)(void*))
{
    ...
    __b_tree_free_bonus(b_tree, free_data);
    free(b_tree);
}
