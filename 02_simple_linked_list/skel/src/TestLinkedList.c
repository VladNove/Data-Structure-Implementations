...
main()
{
    ...
    while (1) {
        ...
        if (strcmp(command, "remove") == 0) {
            ...
            removed_node = ll_remove_nth_node(linkedList, pos);
            if (removed_node) // eliberam memoria nodului scos
            {
        ...
        if (strcmp(command, "addmiddle") == 0) {
            scanf("%s", added_elem);
            nr = strtol(added_elem, &end_ptr, 10);
            if (nr != 0) {
                ll_add_middle_node(linkedList, &nr);
            } else {
                ll_add_middle_node(linkedList, end_ptr);
            }
        }
